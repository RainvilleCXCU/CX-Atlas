...
const BaseLayout: FC<BaseLayoutProps> = ({ props, children = <></>, pageTitle, templateName }) => {
    ...
    let { title = '', content, seo = {}, link = '', featuredImage, databaseId = '', details } = props?.data?.page ?? props?.data?.post ?? props?.data?.posts ?? props?.data?.location ?? props?.data?.category ?? props.data?.postPreview ?? props.data?.locationsPreview ?? {
        title: '',
        seo: {},
        link: '',
        databaseId: ''
