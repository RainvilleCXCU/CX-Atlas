        ...
        <>
            {(!state?.search?.isOpen && alerts && alerts.length > 0) && alerts.map((post, index) => (
                <Fragment key={`${post?.name}-${index}`}>
                    <div id="alert-banner" className={`cx-alert${post.type ? ` cx-alert__type--${post.type}` : ''}${showAlert(post.databaseId) ? ' show': ' hidden'}`}>
                        <button className="cx-alert__close" onClick={closeAlert} data-alert-name={post.databaseId}>&times;</button>
                        <p className="cx-alert__message title"><div>{parseHtml(post.message ?? "")}</div>
                            {post.ctaButtonText &&
